...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    const base = path
      .basename(file.originalname || "study_material", ext)
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      .request()
      .input("Subject", sql.NVarChar(200), subjectKey)
      .input("ClassName", sql.NVarChar(50), classKey)
...
router.post("/", upload.single("file"), async (req, res) => {
  ...
  if (!subject || !className) {
    return res
      .status(400)
      .json({ error: "subject and className are required" });
  }
  if (!req.file) {
